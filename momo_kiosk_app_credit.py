...
APP_NAME = "Food Hub" DATA_FOLDER = "food_hub_data/" BACKUP_FOLDER = os.path.join(DATA_FOLDER, "backups") PASSWORDS = {"Admin": "admin123", "Staff": "staff123"}

DATABASES = { "orders": os.path.join(DATA_FOLDER, "orders.csv"), "credit": os.path.join(DATA_FOLDER, "credit_log.csv"), "inventory": os.path.join(DATA_FOLDER, "inventory.csv") }

MENU = { "Momos": { "Veg Half (6 pcs)": {"price": 80, "cost": 10}, "Veg Full (12 pcs)": {"price": 150, "cost": 20}, "Chicken Half (6 pcs)": {"price": 100, "cost": 17}, "Chicken Full (12 pcs)": {"price": 190, "cost": 34} }, "Sandwich": { "Veg Sandwich": {"price": 60, "cost": 15}, "Cheese Veg Sandwich": {"price": 80, "cost": 25}, "Chicken Sandwich": {"price": 100, "cost": 30} }, "Maggi": { "Plain Maggi": {"price": 40, "cost": 10}, "Veg Maggi": {"price": 60, "cost": 20}, "Cheese Maggi": {"price": 70, "cost": 25}, "Chicken Maggi": {"price": 90, "cost": 30} } }
...
def init_data(): os.makedirs(DATA_FOLDER, exist_ok=True) os.makedirs(BACKUP_FOLDER, exist_ok=True) for path in DATABASES.values(): if not os.path.exists(path): with open(path, "w") as f: f.write("")

init_data()

st.set_page_config(page_title=APP_NAME, layout="wide") st.title(APP_NAME)

if "user_role" not in st.session_state: st.session_state.user_role = None if "authenticated" not in st.session_state: st.session_state.authenticated = False if "orders" not in st.session_state: st.session_state.orders = []
...
with st.sidebar: st.title("Login") if not st.session_state.authenticated: role = st.selectbox("Select Role", ["Admin", "Staff"]) password = st.text_input("Password", type="password") if st.button("Login"): if PASSWORDS.get(role) == password: st.session_state.user_role = role st.session_state.authenticated = True st.success(f"Logged in as {role}") st.rerun() else: st.error("Incorrect password") else: st.success(f"Logged in as {st.session_state.user_role}") if st.button("Logout"): st.session_state.authenticated = False st.session_state.user_role = None st.rerun()
...
if st.session_state.authenticated: st.subheader("Place an Order") customer = st.text_input("Customer Name (optional)", placeholder="Enter name for credit or tracking") category = st.selectbox("Select Category", list(MENU.keys()))
...
if st.session_state.orders:
    ...
    if st.button("Submit & Save Order"):
        df.to_csv(DATABASES["orders"], indeimport streamlit as st
import pandas as pd
from datetime import datetime
import os
import shutil
...
else: st.warning("Please log in using the sidebar to use the app.")

x=False, mode='a', header=not os.path.exists(DATABASES["orders"]))
        if payment_mode == "Credit" and customer:
            credit_df = df[["timestamp", "customer", "total_sale"]].rename(columns={"total_sale": "amount"})
            credit_df.to_csv(DATABASES["credit"], index=False, mode='a', header=not os.path.exists(DATABASES["credit"]))
            st.warning(f"₹{total_sale} added to credit for {customer}")
